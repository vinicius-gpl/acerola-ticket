			...
			<section class="mt-3 grid grid-cols-2 gap-3">
				...
				<AcerolaCard data={{ title: 'Rede' }}>
					...
					<div class="text-muted-foreground mt-2 flex gap-4 text-xs">
						<span
							><span class="bg-chart-5 mr-1 inline-block h-2 w-2 rounded-full"></span>↓ {bytesPerSec(
								netTotals.recv
							)}</span
						>
						<span
							><span class="bg-chart-2 mr-1 inline-block h-2 w-2 rounded-full"></span>↑ {bytesPerSec(
								netTotals.sent
							)}</span
						>
					</div>
				...
				<AcerolaCard data={{ title: 'Inventário' }}>
					<dl class="flex flex-col gap-1 text-xs">
						...
						<div class="flex justify-between">
							...
							<dd class="truncate text-right">{snap.host.cpuModel}</dd>
						</div>
						<div class="flex justify-between">
