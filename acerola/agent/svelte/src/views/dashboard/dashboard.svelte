				...
				{#snippet content()}
					<div class="flex flex-col gap-2 pt-1 text-xs">
						...
						<p class="text-muted-foreground text-[11px] leading-relaxed">
							Para minimizar, feche esta janela. Para encerrar o agente, clique em
							<strong>Sair</strong> no menu da bandeja do Windows.
						</p>
					</div>
				{/snippet}
			</AcerolaPopover>
					...
					<AcerolaCard data={{ title: 'Memória' }} ui={{ class: 'rounded-lg h-full' }}>
						...
						<div class="mt-2">
							<AcerolaSparkline
								...
								ui={{ colorVars: ['--chart-4'], fixedMax: 100, height: 70 }}
							/>
						</div>
						...
						<div class="border-border mt-2 border-t pt-2">
							<AcerolaSegmentedBar
								...
								ui={{ colorVar: '--chart-4', height: 16 }}
							/>

							<div class="text-muted-foreground mt-1.5 flex justify-between text-[11px]">
								<span>Uso atual</span>
								<span>{bytes(snap.memory.usedBytes)} / {bytes(snap.memory.totalBytes)}</span>
							</div>
						</div>
					</AcerolaCard>
				</div>
				...
				<div class="border-border bg-card rounded-xl border p-1">
					<AcerolaCard data={{ title: 'Rede' }} ui={{ class: 'rounded-lg h-full' }}>
						...
						<div class="border-border mt-2 border-t pt-2">
							...
							<div class="border-border mt-2 grid grid-cols-2 gap-x-4 border-t pt-2 text-xs">
								<div class="min-w-0">
									<span class="text-muted-foreground block text-[10px] uppercase">IP local</span>
									<span class="block truncate font-mono font-medium" title={snap.host.localIp}>
										{snap.host.localIp || '—'}
									</span>
								</div>

								<div class="min-w-0">
									<span class="text-muted-foreground block text-[10px] uppercase">MAC</span>
									<span class="block truncate font-mono font-medium" title={snap.host.macAddress}>
						...
						<div class="border-border mt-3 border-t pt-2">
							<div class="flex flex-col gap-1.5">
								{#each snap.disks as disk (disk.mountpoint)}
									<div
										class="grid grid-cols-[3.5rem_minmax(0,1fr)_auto] items-center gap-2 text-xs"
									>
										<AcerolaTooltip data={{ text: `${disk.mountpoint} (${disk.fstype})` }}>
										...
										<AcerolaTooltip
											data={{
												text: `${disk.mountpoint}: ${disk.usedPercent.toFixed(1)}% ocupado · ${bytes(disk.usedBytes)} usados de ${bytes(disk.totalBytes)}`
											}}
										>
											<span class="text-muted-foreground cursor-pointer whitespace-nowrap">
												{bytes(disk.freeBytes)} livres
											</span>
										</AcerolaTooltip>
									</div>
								{/each}
			...
			<section class="mt-3 grid grid-cols-3 gap-3">
				<div class="border-border bg-card col-span-2 rounded-xl border p-1">
					<AcerolaCard data={{ title: 'Processos' }} ui={{ class: 'rounded-lg h-full' }}>
						<AcerolaProcessTable data={{ processes: snap.processes }} />
					</AcerolaCard>
				</div>
