...
describe('ProgressBar', () => {
  ...
  it('shows the percentage and announces the progress to screen readers', () => {
    render(
      <ProgressBar data={{ percentage: 62, done: 8, total: 13 }} ui={{ itemLabel: 'tarefas' }} />,
    );
  ...
  it('shows the count when asked', () => {
    render(<ProgressBar data={{ percentage: 62, done: 8, total: 13 }} ui={{ showCount: true }} />);
  ...
  it('changes the tone by range', () => {
    ...
    rerender(<ProgressBar data={{ percentage: 10, done: 1, total: 13 }} />);
  ...
  it('does not draw an empty red bar when there is nothing to do', () => {
    render(<ProgressBar data={{ percentage: 0, done: 0, total: 0 }} />);
  ...
  it('still draws the bar, empty, for one pending item', () => {
    render(<ProgressBar data={{ percentage: 0, done: 0, total: 1 }} />);

    expect(screen.getByRole('progressbar')).toBeInTheDocument();
  });

  it('clamps a percentage out of range instead of overflowing the bar', () => {
    render(<ProgressBar data={{ percentage: 140, done: 7, total: 5 }} />);
