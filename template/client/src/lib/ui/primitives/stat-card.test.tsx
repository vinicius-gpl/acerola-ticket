...
describe('StatCard', () => {
  it('mostra o rótulo e o número', () => {
    render(<StatCard data={{ label: 'Clientes', value: 560 }} />);
  ...
  it('mostra a ressalva quando o número exclui alguma coisa', () => {
    render(<StatCard data={{ label: 'Ativos', value: 641, hint: 'Exclui 1.067 arquivados' }} />);

    expect(screen.getByText('Exclui 1.067 arquivados')).toBeInTheDocument();
  });

  it('sem ressalva, não desenha linha vazia', () => {
    const { container } = render(<StatCard data={{ label: 'Equipe', value: 22 }} />);
  ...
  it('zero aparece como zero, e não como vazio', () => {
    render(<StatCard data={{ label: 'Inativas', value: 0 }} />);

    expect(screen.getByText('0')).toBeInTheDocument();
  });

  it('carregando esconde o número em vez de mostrar um valor errado', () => {
    render(<StatCard data={{ label: 'Clientes', value: 560 }} state={{ isLoading: true }} />);
  ...
  it('aceita texto no lugar de número, para razões como "13/13"', () => {
    render(<StatCard data={{ label: 'Cadastrados', value: '13/13' }} />);
  ...
  it('com ícone, desenha o quadrado no lugar da barra lateral', () => {
    const { container } = render(
      <StatCard data={{ label: 'Clientes', value: 7 }} ui={{ icon: Building2 }} />,
    );
  ...
  it('sem ícone, continua desenhando a barra lateral', () => {
    const { container } = render(<StatCard data={{ label: 'Clientes', value: 7 }} />);
