/**
 * A MESMA TRAVA do git (`template/scripts/git/branch-guard.sh`), só que ANTES de o Claude
 ...
 *
 * As listas são as mesmas do hook do git (`template/scripts/git/protected-*.txt`): um lugar só.
...
const LISTS_DIR = join(PROJECT_DIR, 'template', 'scripts', 'git');
